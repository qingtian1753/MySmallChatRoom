...
void ChatServer::run()
{
    while(running)
    {
        std::vector<epoll_event>fds(std::move(epoll.wait()));
        for(auto& ev : fds)
        {
            int fd=ev.data.fd;
...
void ChatServer::recvAll(int fd)
{
    ...
    while(1)
    {
        ...
        if(bytes_read>0)
        {
            it->second.read_buffer.append(buffer,bytes_read);
            continue;
        }
        if(bytes_read == 0)
        {
...
void ChatServer::flushOneClient(int fd)
{
    ...
    while(1)
    {
        if(it->second.out_queue.empty() && it->second.write_buffer.empty())
        {
            epoll.modEpollToRead(fd);
            return;
        }
        
        if(it->second.write_buffer.empty() && !it->second.out_queue.empty())
        {
            it->second.write_buffer = it->second.out_queue.front();
            it->second.out_queue.pop();
        }

        std::string& msg = it->second.write_buffer;
        size_t& pos = it->second.write_pos;
...
void ChatServer::handleReadBuffer(int fd)
{
    ...
    {
        std::string msg; 
        if(!protocol::tryParseOne(it->second.read_buffer,msg))
        {
           break;
        }
